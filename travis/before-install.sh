...
wget -O /tmp/chromedriver.zip https://chromedriver.storage.googleapis.com/2.31/chromedriver_linux64.zip
mkdir /tmp/chromedriver
pushd /tmp/chromedriver
unzip /tmp/chromedriver.zip
popd
wget -O /tmp/geckodriver.tar.gz https://github.com/mozilla/geckodriver/releases/download/v0.18.0/geckodriver-v0.18.0-linux64.tar.gz
mkdir /tmp/geckodriver
tar -C /tmp/geckodriver -xzf /tmp/geckodriver.tar.gz
export PATH=$PATH:/tmp/geckodriver:/tmp/chromedriver
export DISPLAY=:99.0
sh -e /etc/init.d/xvfb start
curl -L -o /tmp/selenium-server.jar http://selenium-release.storage.googleapis.com/3.5/selenium-server-standalone-3.5.3.jar
